...
import { ArrowDownCircle, ArrowLeft, Menu, X, LogOut, Search } from 'lucide-react';
...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-brand-light-teal">
      ...
      <header className="py-2 sticky top-0 z-50 bg-brand-light-teal/80 backdrop-blur-md shadow-sm h-[5rem] md:h-[7rem]">
        <div className="wrapper flex justify-between items-center h-full">
          ...
          <div className="flex items-center">
            ...
            >
              {/* Removed mr-3 from Image, spacing handled by nav's margin */}
              <Image src="/logo.png" alt="Sukoon Logo" width={200} height={80} className="w-[130px] h-auto sm:w-[160px] md:w-[180px] lg:w-[200px]" />
            </Link>
          ...
          <div className="hidden md:flex flex-1 items-center justify-center px-2 sm:px-4">
            <div className="w-full max-w-md"> {/* Container to manage search bar width within flex layout */}
              <ProfessionalSearch />
            </div>
          </div>

          {/* Right Group: Auth Links (and Book Now) */}
          <nav className="hidden md:flex items-center space-x-3 sm:space-x-4 md:space-x-6 text-sm md:text-base flex-shrink-0">
            ...
            {currentUser ? (
              <>
                {currentUser.displayName && <span className="text-brand-dark-blue text-xs sm:text-sm md:text-base whitespace-nowrap">Hi, {currentUser.displayName.split(' ')[0]}</span>}
          ...
          <div className="md:hidden flex items-center space-x-3">
            {currentUser ? (
              ...
              <>
                ...
                {/* Login link removed from here, it is now in the mobile navigation menu */}
              </>
            )}
            {/* Mobile Search Icon Button - Moved to the left of Menu icon*/}
            <button 
                onClick={() => {
      ...
      {/* Mobile Search Input Area - Adjusted for fixed dropdown/overlay behavior */}
      {isMobileSearchOpen && (
