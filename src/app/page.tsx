...
const Card: React.FC<CardProps> = ({ title, description, imageUrl, onClick }) => {
  return (
    <motion.div
      className="bg-white/80 p-6 rounded-lg shadow-lg text-center cursor-pointer hover:shadow-xl transition-shadow duration-300 w-64 h-80 flex flex-col items-center justify-center"
      onClick={onClick}
      whileHover={{ scale: 1.05 }}
    >
      <div className="w-64 h-64 relative mb-4 bg-gray-200 rounded-md">
        <Image src={imageUrl} alt={title} layout="fill" objectFit="contain" className="rounded-md" />
      </div>
...
export default function HomePage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-brand-light-teal">
      {/* Header */}
      <header className="py-6 px-8 md:px-16 lg:px-24 flex justify-between items-center sticky top-0 z-50 bg-brand-light-teal/80 backdrop-blur-md shadow-sm h-[6rem]">
      ...
      <main 
        className="flex flex-col items-center justify-center text-center px-4 pt-10 pb-10" 
      ...
      >
        ...
        <p className="text-lg md:text-xl text-brand-text max-w-2xl mb-10">
      ...
      <section className="bg-white px-4 md:px-16 lg:px-24 text-center pt-8 pb-16">
